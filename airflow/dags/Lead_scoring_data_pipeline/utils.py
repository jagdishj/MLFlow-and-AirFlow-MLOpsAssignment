...
def load_data_into_db(db_path, db_file_name,data_directory):
    ...
    cnx.close()
    return "Data is Successfully Loaded into the Table-[loaded_data]."
...
def map_city_tier(db_path, db_file_name,city_tier_mapping):
    ...
    cnx.close()
    return "city_tier mapping is completed on [loaded_data] table."
...
def map_categorical_vars(db_path,db_file_name,list_platform,list_medium,list_source):
    ...
    cnx.close()
    return "categorical_variables_mapped is completed on city_tier_mapped table."
...
def interactions_mapping(db_path,db_file_name,interaction_mapping_file,index_columns):
    ...
    cnx.close()
    return "interactions_mapped successfully on categorical_variables_mapped table."
